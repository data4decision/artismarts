  ...
  const fetchNotifications = async () => {
    try {
      ...
      const customerIds = [...new Set(
        (data || []).map((n: any) => n.data?.customer_id).filter(Boolean)
      )]
      ...
      const formatted = (data || []).map((n: any): NotificationItem => {
        if (n.type === 'admin_message') {
          ...
          const customer = customerMap[n.data?.customer_id] || { name: 'Customer', image: null }
          return {
            id: n.id,
            title: n.title,
            message: n.message,
              ...
              >
                <div className="flex gap-5">
                  ...
                  <div className="flex-shrink-0">
                    {notif.adminImage || notif.customerImage ? (
                      <Image
                        src={notif.adminImage || notif.customerImage || ''}
                        alt="profile"
                        width={56}
                        height={56}
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <p className='font-semibold text-[var(--blue)] text-lg'>
                          {notif.adminName || notif.customerName || 'System'}
                        </p>
