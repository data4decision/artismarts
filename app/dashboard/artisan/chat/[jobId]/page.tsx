...
export default function ArtisanChatPage() {
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setMessages(prev =>
              prev.map(m =>
                m.id === updatedMsg.id
                  ? { ...updatedMsg, is_admin: isAdminMessage(updatedMsg.sender_id) }
                  : m
              )
            )
          }
  ...
  }, [jobId, router, isAdminMessage])

  // ====================== FETCH JOB AND INITIAL MESSAGES ======================
  const fetchJobAndMessages = async () => {
    setLoading(true)
    ...
    try {
      ...
      setCurrentUserId(user.id)   // ← Critical fix
      ...
      if (jobError || !rawJob) throw jobError || new Error('Job not found or not assigned to you')
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (jobError || !jobData?.customer_id) throw jobError || new Error('No customer found')
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map(m =>
          m.id === editingMessageId 
            ? { ...m, content: originalContent, is_edited: false, edited_at: null } 
            : m
        )
      )
      toast.error('Could not update message')
  ...
  useEffect(() => {
    ...
    const markAsSeen = async () => {
      ...
      await supabase
        .from('user_job_read_status')
        .upsert({ user_id: currentUserId, job_id: jobId, last_read_at: now })
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="text-center max-w-md">
          ...
          <Link href="/dashboard/artisan/assigned-jobs" className="inline-flex items-center px-8 py-4 bg-[var(--blue)] text-white rounded-xl hover:bg-[var(--blue)]/90 transition shadow-md">
            <FaArrowLeft className="mr-2" />
            Back to My Jobs
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <header className="bg-gradient-to-r from-[var(--blue)] to-[var(--orange)]/50 text-white px-6 py-4 shadow-lg fixed top-0 left-0 right-0 z-20">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/dashboard/artisan/assigned-jobs" className="hover:opacity-80 transition">
              <FaArrowLeft size={24} />
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold">Chat with Admin - {job.title}</h1>
              <p className="text-sm opacity-90 mt-1">
                Status: {job.status} • Customer: {customer?.first_name ?? ''} {customer?.last_name ?? ''}
              </p>
            </div>
      ...
      <div className="flex-1 overflow-y-auto pt-28 pb-24 px-4 md:px-6 max-w-5xl mx-auto w-full bg-gray-50">
        {messages.length === 0 ? (
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              <div key={msg.id} className={`group flex flex-col relative ${msg.is_admin ? 'items-start' : 'items-end'}`}>
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                  ...
                  {editingMessageId === msg.id ? (
                    <div className="flex flex-col gap-2">
                      ...
                      />
                      <div className="flex gap-3 justify-end">
                        <button onClick={cancelEdit} className="px-4 py-1.5 text-sm text-gray-600 hover:text-gray-800 rounded hover:bg-gray-100">Cancel</button>
                        <button onClick={saveEdit} className="px-4 py-1.5 text-sm font-medium bg-[var(--orange)] text-white rounded hover:bg-orange-600" disabled={!editText.trim()}>Save</button>
                      </div>
                    </div>
                  ) : msg.deleted_at ? (
                    ...
                    <>
                      <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                      <div className="flex items-center justify-end gap-2 text-xs opacity-70 mt-1">
                        {msg.is_edited && <span className="italic">edited • {new Date(msg.edited_at || msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
                ...
                {!msg.is_admin && !msg.deleted_at && editingMessageId !== msg.id && (
                  <div className="relative">
                    <button
                      onClick={() => setOpenMessageId(openMessageId === msg.id ? null : msg.id)}
                      className="absolute bottom-6 right-1 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full"
                      title="More options"
                    >
                    ...
                    {openMessageId === msg.id && (
                      <div className="absolute top-full right-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 py-1 min-w-[140px] z-50">
                        <button onClick={() => { startEdit(msg); setOpenMessageId(null); }} className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-50 text-[var(--blue)]">
                          <FaEdit size={14} /> Edit
                        </button>
                        <button onClick={() => { handleDeleteMessage(msg.id); setOpenMessageId(null); }} className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-50 text-red-600">
            ...
            {otherIsTyping && <div className="flex items-center text-[var(--orange)] text-sm italic pl-4">Admin is typing...</div>}
            <div ref={messagesEndRef} />
          </div>
        )}
      </div>
