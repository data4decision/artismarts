...
//                     <Link
//                       href="/dashboard/customer/messages"
//                       className="inline-flex items-center gap-3 px-8 py-4 bg-[var(--orange)] hover:bg-orange-600 text-white font-bold text-lg rounded-2xl transition shadow-lg hover:shadow-xl w-full justify-center"
//                     >
...
// function getEstimatedTimeRemaining(createdAt: string) {
//   const created = new Date(createdAt)
//   const now = new Date()
//   const diffMs = now.getTime() - created.getTime()
//   const diffHours = Math.floor(diffMs / (1000 * 60 * 60))
                ...
                {requests.map((req) => (
                  ...
                  >
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-wrap items-center justify-between gap-4">
                      ...
                      {/* Chat with Admin button – always visible once request exists */}
                      <Link
                        href="/dashboard/customer/messages"
                        onClick={(e) => e.stopPropagation()}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-8 space-y-8">
                ...
                {/* ... keep budget, location, preferred time, assigned artisan, history sections unchanged ... */}

                {/* Chat button in modal – always visible */}
                <div className="pt-6">
                  <Link
                    href="/dashboard/customer/messages"
...
// Estimated time helper (unchanged)
function getEstimatedTimeRemaining(createdAt: string) {
  const created = new Date(createdAt)
  const now = new Date()
  const diffMs = now.getTime() - created.getTime()
