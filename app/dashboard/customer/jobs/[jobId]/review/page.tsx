...
export default function CustomerJobReviewPage() {
  ...
  const fetchJobAndReview = async () => {
    ...
    try {
      ...
      const { data: jobData, error } = await supabase
        .from('job_requests')
        .select(`
          id, title, description, status, completed_at,
          progress_photo_urls, completion_photo_urls, final_note,
          assigned_artisan_id,
          artisan:assigned_artisan_id (first_name, last_name, profile_image),
          customer_review_rating, customer_review_comment,
          customer_quality_rating, customer_punctuality_rating,
          customer_communication_rating, customer_cleanliness_rating,
          customer_would_hire_again, artisan_notified_of_review
        `)
        .eq('id', jobId)
        .eq('customer_id', user.id)
        .single()
  ...
  try {
    ...
    toast.success('Thank you for rating ArtiSmart! ❤️')
    setShowAppRatingModal(false)
    ...
    toast.error(err.message || 'Failed to submit app rating')
  }
}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="relative flex items-center justify-center">
          <div className="animate-spin rounded-full h-20 w-20 border-4 border-transparent border-t-[var(--orange)]"></div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="text-center max-w-md">
          ...
          <button 
            onClick={() => router.back()}
            className="px-8 py-4 bg-[var(--blue)] text-white rounded-xl hover:bg-blue-700 transition"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto space-y-8">
          ...
          {allPhotos.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {allPhotos.map(({ url, type }, idx) => (
                <div
                  key={idx}
                  className="relative aspect-square rounded-xl overflow-hidden shadow cursor-zoom-in group hover:shadow-2xl transition-all"
        ...
        {/* Review Form - Only show if not already reviewed */}
        {!alreadyReviewed && (
          <div className="bg-white rounded-2xl shadow p-8">
            <h2 className="text-3xl font-bold text-[var(--blue)] mb-8 text-center">
              How was the artisan's work?
            </h2>

            {/* Overall Rating */}
            <div className="mb-12">
              <label className="block text-2xl font-semibold text-[var(--blue)] mb-6 text-center">
                Overall Rating
              </label>
              <div className="flex justify-center gap-4">
                {[1,2,3,4,5].map(star => (
                  ...
                  >
                    <FaStar
                      size={60}
                      className={star <= (hoverOverall || overallRating) ? 'text-[var(--orange)]' : 'text-gray-200'}
                    />
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
              {[
                ...
                <div key={label} className="text-center">
                  <label className="block text-lg font-medium text-[var(--blue)] mb-4">{label}</label>
                  <div className="flex justify-center gap-2">
                    {[1,2,3,4,5].map(s => (
                      <button key={s} onClick={() => setter(s)} className="focus:outline-none">
                        <FaStar size={38} className={s <= value ? 'text-[var(--orange)]' : 'text-gray-200'} />
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="mb-12">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center">
                <button
                  onClick={() => setWouldHireAgain(true)}
                  className={`flex-1 max-w-xs py-5 rounded-2xl border-2 text-lg font-medium transition-all ${
                    wouldHireAgain === true ? 'border-[var(--blue)] bg-blue-50' : 'border-gray-300 hover:border-[var(--blue)]'
                  }`}
                >
                  Yes, definitely
                </button>
                <button
                  onClick={() => setWouldHireAgain(false)}
                  className={`flex-1 max-w-xs py-5 rounded-2xl border-2 text-lg font-medium transition-all ${
                    wouldHireAgain === false ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-red-400'
                  }`}
                >
                  No, probably not
                </button>
            ...
            {/* Comment */}
            <div className="mb-10">
              <label className="block text-xl font-semibold text-[var(--blue)] mb-4">Additional Comments (Optional)</label>
              <textarea
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                placeholder="Tell us about your experience..."
                className="w-full h-40 p-5 border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--orange)]"
              />
            </div>
            ...
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-lg text-white transition-all ${
                submitting || overallRating === 0 ? 'bg-gray-400' : 'bg-[var(--orange)] hover:bg-orange-600'
              }`}
            >
      ...
      {/* ==================== APP RATING MODAL (Auto Show) ==================== */}
      {showAppRatingModal && (
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-10 text-center">
            <h3 className="text-3xl font-bold text-[var(--blue)] mb-3">
              One more thing...
            </h3>
            <p className="text-gray-600 mb-8">
              How would you rate your overall experience with ArtiSmart?
            </p>
              ...
              {[1,2,3,4,5].map((star) => (
                ...
                >
                  <FaStar
                    size={52}
                    className={`${
                      (appHover || appRating) >= star ? 'text-[var(--orange)]' : 'text-gray-200'
                    }`}
                  />
                </button>
              ))}
            ...
            <textarea
              ...
              placeholder="Tell us what you think about the app (optional)"
              className="w-full h-28 p-4 border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--orange)] resize-y mb-8"
            />
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className={`flex-1 py-4 rounded-2xl font-semibold text-white transition ${
                  appRating === 0 ? 'bg-gray-400' : 'bg-[var(--orange)] hover:bg-orange-600'
                }`}
              >
                Submit Rating
              </button>
