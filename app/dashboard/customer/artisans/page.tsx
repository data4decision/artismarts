...
export default function FindArtisansPage() {
  ...
  const fetchArtisans = async () => {
    ...
    try {
      ...
      const artisanIds = profilesData.map(a => a.id)

      // Step 2: Count active jobs for all artisans in one query
      const { data: jobsCountData, error: countError } = await supabase
        .from('job_requests')
        .select('assigned_artisan_id')
        .in('assigned_artisan_id', artisanIds)
      ...
      // Group and count jobs per artisan
      const jobCounts: Record<string, number> = {}
      jobsCountData?.forEach(job => {
        const id = job.assigned_artisan_id
        jobCounts[id] = (jobCounts[id] || 0) + 1
      })

      // Merge counts into profiles
      const transformed = profilesData.map((artisan: any) => ({
        ...artisan,
        active_jobs_count: jobCounts[artisan.id] || 0
      }))

      setArtisans(transformed)
      console.log('✅ Loaded artisans with active jobs:', transformed)
    } catch (err: any) {
      console.error('Fetch error:', err)
      setError(err.message || 'Failed to load artisans')
      toast.error('Failed to load artisans. Please check console.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex justify-center">
          <button
            ...
            className={`px-8 py-4 rounded-xl font-medium text-lg transition flex items-center justify-center gap-3 shadow-md min-w-[220px]
              ${loading ? 'bg-gray-50 cursor-not-allowed text-white' : 'bg-[var(--orange)] hover:bg-orange-600 text-[var(--white)]'}`}
          >
            {loading ? (
              <>
        ...
        {error && !loading && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center max-w-2xl mx-auto">
            ...
            <button onClick={fetchArtisans} className="mt-6 px-8 py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 transition shadow-md">
              Try Again
            </button>
          </div>
        )}
          ...
          <>
            {filteredArtisans.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm border p-12 text-center text-[var(--blue)] max-w-3xl mx-auto">
                ...
                <h3 className="text-xl font-semibold text-[var(--blue)] mb-2">No artisans found</h3>
                <p className="mb-6">
                  {searchQuery ? "Try a different search term or refresh" : "Verified artisans will appear here when available"}
                </p>
                <button onClick={fetchArtisans} className="inline-block px-8 py-4 bg-[var(--blue)] text-[var(--white)] rounded-xl hover:bg-blue-700 transition shadow-md">
                  Refresh List
                </button>
              </div>
            ) : (
                      ...
                      <div className="space-y-3 text-sm text-[var(--blue)]">
                        {artisan.average_rating && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>{artisan.average_rating.toFixed(1)} ({artisan.rating_count || 0} reviews)</span>
                          </div>
                        )}
                        ...
                        {/* Active Jobs Badge - Fiverr Style */}
                        <div className="flex items-center gap-2 pt-2">
                          <div className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-medium border
                            ${activeJobs > 0 
                              ? 'bg-green-50 text-green-700 border-green-200' 
                              : 'bg-gray-100 text-gray-500 border-gray-200'
                            }`}>
                            <FaClock className={activeJobs > 0 ? 'text-green-600' : 'text-gray-400'} />
