...
export default function CustomerJobChatPage() {
  ...
  const typingTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  const isPageVisibleRef = useRef(true)
  const optimisticMessageIds = useRef(new Set<string>()) // Prevents duplicates
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            // FIX: Skip our own messages (already added optimistically)
            if (newMsg.sender_id === currentUserId || optimisticMessageIds.current.has(newMsg.id)) {
              return
            }

            const processedMsg: Message = {
              ...newMsg,
              is_admin: true  // Admin message → show on LEFT
            }
            ...
            setMessages(prev =>
              prev.map(m =>
                m.id === updated.id
                  ? { 
                      ...updated, 
                      is_admin: updated.sender_id !== currentUserId 
                    }
                  : m
              )
            )
  ...
  const handleSendMessage = async () => {
    ...
    const optimisticMsg: Message = {
      ...
      reply_to_id: replyTo?.id || null,
      reply_to_content: replyTo?.content || null,
      is_admin: false   // Your message → show on RIGHT
    }
    ...
    try {
      ...
      optimisticMessageIds.current.add(tempId)  // Prevent duplicate from realtime
      setReplyTo(null)
    } catch (err: any) {
      setMessages(prev => prev.filter(m => m.id !== tempId))
      toast.error('Failed to send message')
