...
interface Job {
  ...
  assigned_artisan_id?: string | null
  customer_id?: string | null
  artisan?: { first_name: string | null; last_name: string | null } | null
...
export default function AdminChatPage() {
  ...
  // ====================== REALTIME (with duplicate protection) ======================
  useEffect(() => {
    if (!job || !currentUserId) return
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'INSERT') {
          ...
          setMessages(prev => {
            ...
            if (tempIndex !== -1) {
              const updated = [...prev]
              updated[tempIndex] = finalMsg
              return updated
            }

            // Normal insert (from other side)
          ...
          setMessages(prev =>
            prev.map(m =>
              m.id === updated.id
                ? { ...updated, is_admin: isAdminMessage(updated.sender_id) }
                : m
            )
          )
        }
  ...
  }, [job, currentUserId, jobId, isAdminMessage])
  ...
  const fetchJobAndMessages = async () => {
    ...
    try {
      ...
      const { data: rawJob } = await supabase
        .from('job_requests')
        .select(`
          id, title, status, assigned_artisan_id, customer_id,
          artisan:assigned_artisan_id (first_name, last_name),
          customer:customer_id (first_name, last_name)
        `)
        .eq('id', jobId)
  ...
  const handleSendMessage = async () => {
    if (!newMessage.trim() || sending || !currentUserId || !job) return
    ...
    // Optimistic message (appears immediately on the right)
    const optimisticMsg: Message = {
      id: tempId,
      job_id: isArtisanChat ? jobId : undefined,
      job_request_id: !isArtisanChat ? jobId : undefined,
      ...
      reply_to_sender: replyTo ? (isArtisanChat ? 'You' : 'Admin') : undefined
    }
    ...
    try {
      ...
      if (error) throw error
      // Realtime listener will replace temp message with real one
    } catch (err: any) {
  ...
  // Edit, Delete, Reply functions (unchanged logic)
  const startEdit = (msg: Message) => {
    if (msg.sender_id !== currentUserId || msg.deleted_at) return
    setEditingMessageId(msg.id)
    setEditText(msg.content)
  ...
  const saveEdit = async () => {
    ...
    setMessages(prev => prev.map(m =>
      m.id === editingMessageId ? { ...m, content: editText.trim(), is_edited: true, edited_at: new Date().toISOString() } : m
    ))
  ...
  useEffect(() => {
    ...
    const markAsSeen = async () => {
      ...
      const now = new Date().toISOString()
      await supabase.from(table).update({ seen_at: now }).in('id', unseen.map(m => m.id))

      setMessages(prev => prev.map(m => (m.seen_at || m.is_admin) ? m : { ...m, seen_at: now }))

      await supabase.from('user_job_read_status').upsert({
        user_id: currentUserId,
        job_id: isArtisanChat ? jobId : null,
        job_request_id: !isArtisanChat ? jobId : null,
        last_read_at: now
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="text-center max-w-md">
          ...
          <Link href="/admin-dashboard/messages" className="inline-flex items-center px-8 py-4 bg-[var(--blue)] text-white rounded-xl hover:bg-blue-700 transition shadow-md">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col relative">
      ...
      <header className="bg-gradient-to-r from-[var(--blue)] to-[var(--orange)] text-white px-6 py-4 shadow-lg fixed top-15 left-0 right-0 w-full z-20">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold">Chat with {recipientType} - {job.title}</h1>
              <p className="text-sm opacity-90 mt-1">{recipientName}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pt-28 pb-32 px-4 md:px-6 max-w-5xl mx-auto w-full bg-gray-50">
        {messages.length === 0 ? (
          ...
          <div className="space-y-4 pb-20">
            {messages.map((msg) => (
              ...
              >
                <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${
                  msg.is_admin ? 'bg-[var(--orange)] text-white rounded-br-none' : 'bg-gray-200 text-gray-900 rounded-bl-none'
                } ${msg.deleted_at ? 'opacity-60' : ''}`}>

                  {msg.reply_to_id && msg.reply_to_content && (
                    <div className="mb-2 pl-3 border-l-4 border-[var(--orange)] text-xs opacity-80">
                      <p className="font-medium">Replying to {msg.reply_to_sender}:</p>
                      <p className="line-clamp-1">{msg.reply_to_content}</p>
                    </div>
                  )}
            ...
            {otherIsTyping && <div className="flex items-center text-gray-500 text-sm italic pl-4">{recipientType} is typing...</div>}
            <div ref={messagesEndRef} />
          </div>
        )}
      </main>
        ...
        <div className="max-w-5xl mx-auto flex flex-col gap-2">
          {replyTo && (
            <div className="flex items-center gap-3 bg-gray-100 px-4 py-2 rounded-lg text-sm">
              <FaReply className="text-[var(--orange)]" />
              <div className="flex-1 truncate">
                Replying to: {replyTo.content.slice(0, 80)}{replyTo.content.length > 80 ? '...' : ''}
              </div>
              <button onClick={cancelReply} className="text-gray-600 hover:text-red-600"><FaTimes /></button>
            </div>
          )}
      ...
      {/* Context Menu */}
      {openMessageId && (
        <div className="fixed bg-white shadow-xl rounded-lg py-2 min-w-[160px] z-50 border border-gray-200" 
             style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
          <button onClick={() => { const msg = messages.find(m => m.id === openMessageId); if (msg) handleReply(msg) }} 
            className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-100 text-[var(--blue)]">
          ...
          <button onClick={() => { 
            ...
            toast.success('Copied')
            setOpenMessageId(null)
          }} className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-100">
            <FaCopy size={14} /> Copy
          </button>
          <button onClick={() => { const msg = messages.find(m => m.id === openMessageId); if (msg) startEdit(msg) }} 
            className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-50 text-[var(--blue)]">
            <FaEdit size={14} /> Edit
          </button>
          <button onClick={() => { handleDeleteMessage(openMessageId); setOpenMessageId(null) }} 
            className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-gray-50 text-red-600">
