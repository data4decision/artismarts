...
//   }, [messages, jobRequestId, loading, currentUserId])
...
//     if (!trimmed || sending || !currentUserId || !jobRequestId) return
...
//           <Link href="/dashboard/admin/jobs" className="inline-flex items-center px-8 py-4 bg-[var(--blue)] text-white rounded-xl hover:bg-blue-700">
...
//           </button>
//           <div>
//             <h1 className="text-xl md:text-2xl font-bold">Chat with Customer - {job.title}</h1>
...
//                           className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--orange)] text-sm"
//                           rows={3}
//                           autoFocus
//                         />
...
//                             className="px-5 py-2 bg-[var(--orange)] text-white rounded-lg hover:bg-orange-600 font-medium disabled:opacity-50 flex items-center gap-2"
//                           >
//                             Save
//                           </button>
//                         </div>
...
//                     {!msg.deleted_at && !editingMessageId && (
...
//             {otherIsTyping && <div className="pl-4 text-[var(--orange)] text-sm italic">Customer is typing...</div>}
...
//         >
//           <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left" onClick={() => {
...
//           <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left text-blue-600" onClick={() => {
//             const msg = messages.find(m => m.id === openMessageId)
...
//           <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left text-red-600" onClick={() => handleDeleteMessage(openMessageId)}>
//             <FaTrash /> Delete
//           </button>
//         </div>
//       )}
...
export default function AdminCustomerChatPage() {
  ...
  const isAdminMessage = useCallback((senderId: string): boolean => {
    return senderId === currentUserIdRef.current
  }, [])
  ...
  // ====================== REALTIME - Handle both INSERT and UPDATE ======================
  useEffect(() => {
    if (!jobRequestId || !currentUserId) return
      ...
      .on(
        ...
        (payload) => {
          ...
          setMessages(prev => {
            ...
            // Replace optimistic temp message (your own message)
            ...
            const processedMsg: Message = {
              ...newMsg,
              is_admin: isAdminMessage(newMsg.sender_id)
            }
            ...
            // Add new message (customer message or any missed insert)
    ...
    const typingChannel = supabase
      .channel(`typing:${jobRequestId}`)
      .on('broadcast', { event: 'typing' }, (payload) => {
        const { is_typing, user_id } = payload.payload as any
        if (user_id !== currentUserId) setOtherIsTyping(is_typing)
      })
      .subscribe()
  ...
  }, [jobRequestId, currentUserId, isAdminMessage])
  ...
  const handleSendMessage = async () => {
    const trimmed = newMessage.trim()
    if (!trimmed || sending || !currentUserId || !jobRequestId || !job?.customer_id) return
  ...
  // Edit, Delete, Reply functions (same as before)
  const startEdit = (msg: Message) => {
    if (msg.sender_id !== currentUserId || msg.deleted_at) return
    setEditingMessageId(msg.id)
    setEditText(msg.content)
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col relative">
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm ${
                    ...
                    {msg.deleted_at ? (
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        />
                        <div className="flex gap-3 justify-end text-sm">
                          <button onClick={cancelEdit} className="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium">Cancel</button>
                          <button onClick={saveEdit} disabled={!editText.trim()} className="px-5 py-2 bg-[var(--orange)] text-white rounded-lg hover:bg-orange-600 font-medium disabled:opacity-50">Save</button>
                        </div>
                      </div>
                    ) : (
                      <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
        ...
        <div className="max-w-5xl mx-auto">
          {replyTo && (
            <div className="mb-3 bg-gray-100 px-4 py-2 rounded-lg flex items-center gap-3 text-sm">
              ...
              <button onClick={cancelReply} className="text-gray-500 hover:text-red-600">
                <FaTimes />
              </button>
            </div>
          )}
      ...
      {/* Context Menu */}
      {openMessageId && (
        <div className="fixed bg-white shadow-2xl rounded-xl py-2 min-w-[160px] z-50 border border-gray-200" style={{ bottom: '100px', right: '16px' }}>
          <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left" onClick={() => { const msg = messages.find(m => m.id === openMessageId); if (msg) { setReplyTo(msg); setOpenMessageId(null); } }}>
            <FaReply className="text-blue-600" /> Reply
          </button>
          <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left" onClick={() => { const msg = messages.find(m => m.id === openMessageId); if (msg) handleCopy(msg.content); }}>
            <FaCopy className="text-gray-700" /> Copy
          </button>
          <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left text-blue-600" onClick={() => { const msg = messages.find(m => m.id === openMessageId); if (msg) startEdit(msg); }}>
            <FaEdit /> Edit
          </button>
          <button className="flex items-center gap-3 w-full px-5 py-2.5 hover:bg-gray-50 text-left text-red-600" onClick={() => handleDeleteMessage(openMessageId)}>
            <FaTrash /> Delete
