...
interface ActiveJob {
  ...
  status: string
  created_at: string
  assigned_at?: string | null   // if you have this column
  location: string
}
...
export default function AdminActiveJobs() {
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    if (filter === 'in_progress') return ['in_progress', 'completed_pending_review'].includes(job.status)
    return true // all
  })
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[var(--blue)]">
              Admin – Active & Assigned Jobs
            </h1>
            <p className="mt-2 text-gray-600">
              Jobs currently assigned or being worked on
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 bg-white rounded-xl shadow-sm border px-4 py-2">
              ...
              >
                ...
                <option value="in_progress">In Progress / Pending Review</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <>
            {filteredJobs.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm border p-12 text-center text-gray-500">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">
                  No active or assigned jobs right now
                </h3>
                <p className="mb-6">
                  When a job is assigned or started by an artisan, it will appear here.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="text-lg font-semibold text-[var(--blue)] line-clamp-1">
                          {job.title}
                        </h3>
                      ...
                      <Link
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--blue)] hover:bg-blue-700 text-white rounded-xl transition font-medium"
                      >
                        <FaEye size={16} />
                        View Full Details
                      </Link>
